...
use Hasantayyar\Osbib\Create\SESSION;
use Hasantayyar\Osbib\Create\MESSAGES;
use Hasantayyar\Osbib\Create\SUCCESS;
use Hasantayyar\Osbib\Create\ERRORS;
use Hasantayyar\Osbib\LOADSTYLE;
use Hasantayyar\Osbib\Create\MISC;
use Hasantayyar\Osbib\Create\FORM;
use Hasantayyar\Osbib\Create\TABLE;
...
// Add a style - display options.
    function addInit($error = FALSE) {
        $pString = MISC::h($this->messages->text("heading", "styles", " (" . $this->messages->text("style", "addLabel") . ")"), FALSE, 3);
        if ($error)
...
// display styles for editing
    function editInit($error = FALSE) {
        $pString = MISC::h($this->messages->text("heading", "styles", " (" . $this->messages->text("style", "editLabel") . ")"), FALSE, 3);
    ...
    function arrayToTemplate($types, $footnote = FALSE) {
        ...
        foreach ($types as $resourceArray) {
            if ($footnote && ($resourceArray['_NAME'] != 'resource')) {
                $this->session->setVar("footnote_" . $resourceArray['_NAME'], base64_encode(htmlspecialchars($resourceArray['_DATA'])));
                continue;
            }
...
//						$empty = TRUE;
                    ...
                    foreach ($array['_ELEMENTS'] as $elements) {
                        if ($array['_NAME'] == 'independent') {
                            ...
                            $temp[$array['_NAME']][$split[1]] = $elements['_DATA'];
                        } else
                            $temp[$array['_NAME']][$elements['_NAME']] = $elements['_DATA'];
                    }
                }
            }
//			if($empty)
...
//			}
            ...
            foreach ($temp as $key => $value) {
                ...
                $pre = $post = $dependentPre = $dependentPost = $dependentPreAlternative = $dependentPostAlternative = $singular = $plural = $string = FALSE;
    ...
    function writeSessionRewriteCreators($type, $array) {
        foreach ($this->creators as $creatorField) {
            ...
            if (array_key_exists($name, $array['_ATTRIBUTES'])) {
                ...
                $this->session->setVar($sessionKey, base64_encode(htmlspecialchars($array['_ATTRIBUTES'][$name])));
            }
...
// display groups for copying and making a new style
    function copyInit($error = FALSE) {
        $pString = MISC::h($this->messages->text("heading", "styles", " (" . $this->messages->text("style", "copyLabel") . ")"), FALSE, 3);
...
// 1st., creator style
        ...
        $td = MISC::b($this->messages->text("cite", "creatorStyle")) . MISC::br() .
                FORM::selectedBoxValue($this->messages->text("style", "creatorFirstStyle"), "cite_creatorStyle", $exampleName, $firstStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorOthers"), "cite_creatorOtherStyle", $exampleName, $otherStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= $this->messages->text("cite", "useInitials") . ' ' . FORM::checkbox(FALSE, "cite_useInitials", $useInitials);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorInitials"), "cite_creatorInitials", $exampleInitials, $initials, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorFirstName"), "cite_creatorFirstName", $example, $firstName, 2);
        $uppercase = base64_decode($this->session->getVar("cite_creatorUppercase")) ?
                TRUE : FALSE;
        $td .= MISC::P(FORM::checkbox($this->messages->text("style", "uppercaseCreator"), "cite_creatorUppercase", $uppercase));
        $pString .= TABLE::td($td);
// Delimiters
        ...
        $td = MISC::b($this->messages->text("cite", "creatorSep")) .
                ...
                FORM::textInput(FALSE, "cite_creatorSepFirstBetween", $betweenFirst, 7, 255) . MISC::br() .
...
// List abbreviation
        ...
        $td .= MISC::b($this->messages->text("cite", "creatorList")) .
                MISC::p(FORM::selectedBoxValue(FALSE, "cite_creatorList", $example, $list, 2) . MISC::br() .
        ...
        $td .= MISC::b($this->messages->text("cite", "creatorListSubsequent")) .
                MISC::p(FORM::selectedBoxValue(FALSE, "cite_creatorListSubsequent", $example, $list, 2) . MISC::br() .
                        $this->messages->text("style", "creatorListIf") . ' ' .
...
// Miscellaneous citation formatting
        ...
        $td .= MISC::p(FORM::textInput($this->messages->text("cite", "replaceYear"), "cite_replaceYear", $replaceYear, 10, 255));
...
// Subsequent citations by same author(s)
        ...
        $td = FORM::selectedBoxValue($this->messages->text("style", "pageFormat"), "cite_pageFormat", $example, $input, 3);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $example = array("1998", "'98", "98");
        $year = base64_decode($this->session->getVar("cite_yearFormat"));
        $td .= FORM::selectedBoxValue($this->messages->text("cite", "yearFormat"), "cite_yearFormat", $example, $year, 3);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
...
// Ambiguous citations
        ...
        $td .= MISC::p(FORM::selectedBoxValue(MISC::b($this->messages->text("cite", "ambiguous")), "cite_ambiguous", $example, $ambiguous, 3));
        $availableFields = join(', ', $this->map->citation);
        $td .= $this->messages->text("cite", "template") . ' ' .
...
// Endnote style citations
        ...
        $td .= MISC::p(FORM::selectedBoxValue($this->messages->text("cite", "endnoteStyle"), "cite_endnoteStyle", $endnoteStyleArray, $endnoteStyle, 3));
...
// bibliography order
        ...
        $pString .= TABLE::td($heading . FORM::selectedBoxValue($this->messages->text("powerSearch", "order1"), "cite_order1", $orderArray, $order1, 3) . MISC::p($radio));
        ...
        $pString .= TABLE::td(FORM::selectedBoxValue($this->messages->text("powerSearch", "order2"), "cite_order2", $orderArray, $order2, 3) . MISC::p($radio), FALSE, FALSE, "bottom");
        ...
        $pString .= TABLE::td(FORM::selectedBoxValue($this->messages->text("powerSearch", "order3"), "cite_order3", $orderArray, $order3, 3) . MISC::p($radio), FALSE, FALSE, "bottom");
        $pString .= TABLE::trEnd();
        $pString .= TABLE::tableEnd();
        $pString .= MISC::br() . "&nbsp;" . MISC::br();
        $pString .= TABLE::tdEnd() . TABLE::trEnd() . TABLE::trStart() . TABLE::tdStart();
...
// display the style form for both adding and editing
    function displayStyleForm($type) {
        ...
        if ($type == 'add')
            $pString .= TABLE::td(FORM::textInput($this->messages->text("style", "shortName"), "styleShortName", $input, 20, 255) . " " . MISC::span('*', 'required') .
            ...
            $pString .= FORM::hidden("editStyleFile", $this->vars['editStyleFile']) .
                    ...
                    TABLE::td(MISC::b($this->vars['editStyleFile'] . ":&nbsp;&nbsp;"), FALSE, FALSE, "top");
        else // copy
            $pString .= TABLE::td(FORM::textInput($this->messages->text("style", "shortName"), "styleShortName", $input, 20, 255) . " " . MISC::span('*', 'required') .
                            MISC::br() . $this->messages->text("hint", "styleShortName"));
        $input = stripslashes(base64_decode($this->session->getVar("style_longName")));
        $pString .= TABLE::td(FORM::textInput($this->messages->text("style", "longName"), "styleLongName", $input, 50, 255) . " " . MISC::span('*', 'required'));
        $input = base64_decode($this->session->getVar("cite_citationStyle"));
        $example = array($this->messages->text("cite", "citationFormatInText"),
            $this->messages->text("cite", "citationFormatEndnote"));
        $pString .= TABLE::td(FORM::selectedBoxValue($this->messages->text("cite", "citationFormat"), "cite_citationStyle", $example, $input, 2) . " " . MISC::span('*', 'required'));
...
// Editor replacements
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "editorSwitchHead")) . MISC::br() .
                        FORM::selectedBoxValue($this->messages->text("style", "editorSwitch"), "style_editorSwitch", $example, $switch, 2));
        $pString .= TABLE::td(
                        FORM::textInput($this->messages->text("style", "editorSwitchIfYes"), "style_editorSwitchIfYes", $editorSwitchIfYes, 30, 255), FALSE, FALSE, "bottom");
        $pString .= TABLE::trEnd();
        $pString .= TABLE::tableEnd();
        $pString .= TABLE::tdEnd() . TABLE::trEnd() . TABLE::trStart() . TABLE::tdStart();
        $pString .= MISC::br() . "&nbsp;" . MISC::br();
// Title capitalization, edition, day and month, runningTime and page formats
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "dayFormat")) . MISC::br() .
                        ...
                        MISC::P(FORM::checkbox($this->messages->text("style", "dayLeadingZero"), "style_dayLeadingZero", $leadingZero)));
        ...
        $pString .= TABLE::td(FORM::selectedBoxValue($this->messages->text("style", "dateMonthNoDay"), "style_dateMonthNoDay", $example, $input, 2) . MISC::br() .
...
// Date range formatting
        ...
        $input = stripslashes(base64_decode($this->session->getVar("style_dateRangeDelimit1")));
        $pString .= TABLE::td(FORM::textInput($this->messages->text("style", "dateRangeDelimit1"), "style_dateRangeDelimit1", $input, 6, 255));
        $input = base64_decode($this->session->getVar("style_dateRangeDelimit2"));
        $pString .= TABLE::td(FORM::textInput($this->messages->text("style", "dateRangeDelimit2"), "style_dateRangeDelimit2", $input, 6, 255));
        $pString .= TABLE::trEnd();
        $pString .= TABLE::trStart();
        ...
        $pString .= TABLE::td(FORM::selectedBoxValue($this->messages->text("style", "dateRangeSameMonth"), "style_dateRangeSameMonth", $example, $input, 2), FALSE, FALSE, FALSE, 2);
        $pString .= TABLE::trEnd();
        $pString .= TABLE::tableEnd();
        $pString .= TABLE::tdEnd() . TABLE::trEnd() . TABLE::trStart() . TABLE::tdStart();
        $pString .= MISC::br() . "&nbsp;" . MISC::br();
...
// Resource types
        foreach ($types as $key) {
            if (($key == 'genericBook') || ($key == 'genericArticle') || ($key == 'genericMisc')) {
                ...
                $fallback = FORM::selectedBoxValue($this->messages->text("style", "fallback"), $formElementName, $generic, $input, 3);
// Replacement citation template for in-text citation for this type
                ...
                $citationString = MISC::p(FORM::textInput($this->messages->text("cite", "typeReplace"), $citationStringName, $input, 60, 255) . $checkBox . MISC::br() .
...
// Footnote template
            ...
            $footnoteTemplate = MISC::p(FORM::textareaInput($this->messages->text("cite", "footnoteTemplate"), $footnoteTemplateName, $input, 80, 3));
            ...
            $pString .= TABLE::td(FORM::textareaInput($heading, $keyName, $input, 80, 3) . $preview . $footnoteTemplate .
...
// 1st., creator style
        ...
        $td = MISC::b($this->messages->text("style", "primaryCreatorStyle")) . MISC::br() .
                FORM::selectedBoxValue($this->messages->text("style", "creatorFirstStyle"), $prefix . "_primaryCreatorFirstStyle", $exampleName, $firstStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorOthers"), $prefix . "_primaryCreatorOtherStyle", $exampleName, $otherStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorInitials"), $prefix . "_primaryCreatorInitials", $exampleInitials, $initials, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorFirstName"), $prefix . "_primaryCreatorFirstName", $example, $firstName, 2);
        $uppercase = base64_decode($this->session->getVar($prefix . "_primaryCreatorUppercase")) ?
                TRUE : FALSE;
        $td .= MISC::P(FORM::checkbox($this->messages->text("style", "uppercaseCreator"), $prefix . "_primaryCreatorUppercase", $uppercase));
        ...
        $td .= FORM::selectedBoxValue($this->messages->text("style", "repeatCreators"), $prefix . "_primaryCreatorRepeat", $exampleRepeat, $repeat, 3) . MISC::br();
...
// Other creators (editors, translators etc.)
        ...
        $td = MISC::b($this->messages->text("style", "otherCreatorStyle")) . MISC::br() .
                FORM::selectedBoxValue($this->messages->text("style", "creatorFirstStyle"), $prefix . "_otherCreatorFirstStyle", $exampleName, $firstStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorOthers"), $prefix . "_otherCreatorOtherStyle", $exampleName, $otherStyle, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorInitials"), $prefix . "_otherCreatorInitials", $exampleInitials, $initials, 4);
        $td .= MISC::br() . "&nbsp;" . MISC::br();
        $td .= FORM::selectedBoxValue($this->messages->text("style", "creatorFirstName"), $prefix . "_otherCreatorFirstName", $example, $firstName, 2);
        $uppercase = base64_decode($this->session->getVar($prefix . "_otherCreatorUppercase")) ?
                TRUE : FALSE;
        $td .= MISC::P(FORM::checkbox($this->messages->text("style", "uppercaseCreator"), $prefix . "_otherCreatorUppercase", $uppercase));
        $pString .= TABLE::td($td);
//		}
        $pString .= TABLE::trEnd();
        $pString .= TABLE::tableEnd();
...
// 2nd., creator delimiters
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "primaryCreatorSep")) .
                        ...
                        MISC::br() . MISC::p($this->messages->text("style", "sepCreatorsNext") . MISC::br() .
                                ...
                                $this->messages->text("style", "creatorSepLast") . "&nbsp;" .
                                FORM::textInput(FALSE, $prefix . "_primaryCreatorSepNextLast", $last, 7, 255)), FALSE, FALSE, "bottom");
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "otherCreatorSep")) .
                        ...
                        MISC::p($this->messages->text("style", "sepCreatorsNext") . MISC::br() .
                                ...
                                $this->messages->text("style", "creatorSepLast") . "&nbsp;" .
                                FORM::textInput(FALSE, $prefix . "_otherCreatorSepNextLast", $last, 7, 255)), FALSE, FALSE, "bottom");
        $pString .= TABLE::trEnd();
        $pString .= TABLE::tableEnd();
        $pString .= TABLE::tdEnd() . TABLE::trEnd() . TABLE::trStart() . TABLE::tdStart();
        $pString .= MISC::br() . "&nbsp;" . MISC::br();
...
// 3rd., creator list limits
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "primaryCreatorList")) . MISC::br() .
                        FORM::selectedBoxValue(FALSE, $prefix . "_primaryCreatorList", $example, $list, 2) . MISC::br() .
                        $this->messages->text("style", "creatorListIf") . ' ' .
        ...
        $pString .= TABLE::td(MISC::b($this->messages->text("style", "otherCreatorList")) . MISC::br() .
                        FORM::selectedBoxValue(FALSE, $prefix . "_otherCreatorList", $example, $list, 2) . MISC::br() .
                        $this->messages->text("style", "creatorListIf") . ' ' .
...
// Re-write creator(s) portion of templates to handle styles such as DIN 1505.
    function rewriteCreators($key, $availableFields) {
        ...
        foreach ($fields as $creatorField => $value) {
            ...
            $first = TABLE::td(MISC::p(FORM::textInput($this->messages->text("style", "rewriteCreator2"), $formString, $string, 20, 255) . $firstCheckbox, "small"), FALSE, FALSE, "bottom");
            ...
            $remainderCheckbox .= ",&nbsp;&nbsp;&nbsp;" . $this->messages->text("style", "rewriteCreator5") .
                    "&nbsp;" . FORM::checkbox(FALSE, $formCheckbox, $checkbox);
            $remainder = TABLE::td(MISC::p(FORM::textInput($this->messages->text("style", "rewriteCreator3"), $formString, $string, 20, 255) . $remainderCheckbox, "small"), FALSE, FALSE, "bottom");
...
// Loop each field string
        ...
        foreach ($subjectArray as $subject) {
            ++$subjectIndex;
            $dependentPre = $dependentPost = $dependentPreAlternative = $dependentPostAlternative = $singular = $plural = FALSE;
...
// If $size % 2 == 0 and exists 'independent_0' and 'independent_' . $index, these are preliminaryText and ultimate
            ...
            else if (!($size % 2) && array_key_exists('independent_0', $independent) && array_key_exists('independent_' . $index, $independent)) {
                $final['preliminaryText'] = array_shift($independent);
                $final['ultimate'] = array_pop($independent);
            }
            $size = sizeof($independent);
