...
// Constructor
    function __construct($db, $output, $export = FALSE, $style = FALSE) {
        $this->db = $db;
...
// load, parse and store to session
        if ((!$styleInfo || !$styleCommon || !$styleTypes) || ($styleInfo != $this->setupStyle)) {
            list($info, $citation, $styleCommon, $styleTypes) = $this->bibformat->loadStyle("styles/bibliography/", $this->setupStyle);
...
// proceedings_article can have publisher as well as organiser/location. Publisher is in 'miscField1'
            if (($type == 'proceedings_article') && $this->row['miscField1']) {
                $recordset = $this->db->select(array("WKX_publisher"), array("publisherName", "publisherLocation"), " WHERE " . $this->db->formatField('id') . "=" .
...
// get names from database for creator, editor, translator etc.
    function grabNames($nameType) {
        ...
        $recordset = $this->db->select(array("WKX_creator"), array("surname", "firstname",
            "initials", "prefix", "id"), " WHERE " . join(" OR ", $conditions));
