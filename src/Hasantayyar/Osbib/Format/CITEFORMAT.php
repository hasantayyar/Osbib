...
namespace Hasantayyar\Osbib\Format;
...
class CITEFORMAT {
     ...
     */
    function __construct(&$bibStyle, $bibStyleProcess, $dir = FALSE, $utf8Dir = FALSE) {
...
//05/05/2005 G.GARDEY: add a last "/" to $dir if not present.
        ...
        $this->citationIds = $this->creatorIds = $this->consecutiveCreatorSep = $this->endnoteStringArray = $this->creators = $this->endnoteCitations = $this->endnoteSameIdsArray = $this->inTextDoneIds = $this->endnotes = $this->opCit = $this->rtfDoneIds = $this->intextBibliography = $this->creatorSurnames = array();
...
// If endnote-style citations, need to ensure we get endnote references with BBCode intact and not parsed by bibformat()
    ...
    function citationToArrayEndnoteStyle() {
        ...
        $temp = PARSESTYLE::parseStringToArray('citationEndnoteInText', trim($this->style['templateEndnoteInText']), $this->styleMap);
// Ensure we have only valid fields.
        ...
        $temp = PARSESTYLE::parseStringToArray('citationEndnote', trim($this->style['templateEndnote']), $this->styleMap);
// Ensure we have only valid fields.
        foreach ($temp as $field => $value) {
...
// Is this citation in the same sentence as the previous citation and for the same resource?
        ...
        $etAlEnd = $surnameEtAl = $possessiveEnd = $possessive1 = $possessive2 = $this->citationInSameSentence = FALSE;
        $storedSurname = $this->item['firstCreatorSurname'];
        $text = str_replace("&nbsp;", " ", $text);
        $split = explode('. ', preg_replace("/\[.*\]|\[\/.*\]|<.*[>]+/Us", "", $text)); // strip BBCode and HTML temporarily
        ...
        foreach ($this->items as $count => $item) {
            if (($this->style['ambiguous'] == 1) &&
                ...
                foreach ($this->years as $yearIndex => $year) {
                    if (array_key_exists('creatorIds', $item) && array_key_exists($yearIndex, $this->creatorIds) && ($year == $item['year']) // same year
...
//print_r($value); print "<P>";
            ...
            if ($this->matchNameSplit && ($key == 'pages')) { // If pages split has occurred, remove dependencies for pages.
                ...
                if ($nextFieldExists && array_key_exists('dependentPost', $value))
                    ...
                    $post = preg_replace("/__DEPENDENT_ON_NEXT_FIELD__/", $value['dependentPostAlternative'], $post);
                else
...
// cache surnames
        ...
        foreach ($creators as $creatorIndex => $creator) {
            ...
            if ($style[$nameStyle] == 0) // Joe Bloggs
                ...
                $nameString = stripslashes($creator['prefix']) . ' ' .
                ...
                if (array_key_exists('useInitials', $this->style)) {
                    foreach ($surnames as $surnameIndex => $surname) {
                        if (($creator['surname'] == $surname) && ($surnameIndex != $creatorIndex)) {
                            if ($style[$list] && $style[$limit] && ($surnameIndex < $style[$moreThan]) && ($creatorIndex < $style[$moreThan])) {
                                $useInitials = TRUE;
                                break;
                            }
                        }
...
// There is a 'u' UTF-8 parameter for preg_xxx but it doesn't work.
            ...
            while (preg_match($match, $pString)) {
                $array = preg_split("/(.*)$delimitLeft(.*)$delimitRight(.*)/U", $pString, 2, PREG_SPLIT_DELIM_CAPTURE);
                /**
                ...
                if ($this->style['endnoteStyle'] == 1) { // Endnotes, same ids
                    foreach (array_unique($ids) as $id) {
                        if (!array_key_exists($id, $this->endnoteCitations)) // don't have this one
                            $this->endnoteCitations[$id] = $this->endnoteRemovePunc($this->bibStyle->$process($rows[$id]));
                    ...
                    foreach ($ids as $id) {
                        ...
                        $this->endnoteCitations[] = $this->endnoteRemovePunc($this->bibStyle->$process($rows[$id]));
                        if (!array_key_exists($id, $this->endnoteSameIdsArray)) {
                            $this->endnoteSameIdsArray[$id] = $endnoteSameIds;
                            ++$endnoteSameIds;
                        }
...
// If footnotes, uses 'pages' formatting from footnote template
                if ($this->style['citationStyle'] && ($this->style['endnoteStyle'] == 2)) {
                    ...
                    if (array_key_exists('pages', $this->bibStyle->bibformat->$type) && array_key_exists('pages', $this->templateEndnote))
