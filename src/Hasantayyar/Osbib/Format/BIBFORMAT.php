...
use Hasantayyar\Osbib\Format\BibtextParse\PARSEENTRIES;
use Hasantayyar\Osbib\Format\BibtextParse\PARSECREATORS;
use Hasantayyar\Osbib\Format\BibtextParse\PARSEMONTH;
use Hasantayyar\Osbib\Format\BibtextParse\PARSEPAGE;
use Hasantayyar\Osbib\STYLEMAPBIBTEX;
...
// WIKINDX-specific
    ...
    function loadStyle($stylePath, $style) {
        ...
        $parseXML = new \Hasantayyar\Osbib\PARSEXML($this);
...
// Match $itemArray[$lastFieldKey] with $pre
            if (($lastFieldKey !== FALSE) && $pre && preg_match("/^[.,;:?!]/", $pre) &&
                    preg_match("/([.,;:?!])(\[.*?[\]]+)*$|([.,;:?!])$/", $itemArray[$lastFieldKey], $capture, PREG_OFFSET_CAPTURE))
// There is punctuation in post either immediately following BBCode formatting or at the start of the string.
...
// There is a 'u' UTF-8 parameter for preg_xxx but it doesn't work.
    ...
    function formatDate($startDay, $startMonth, $endDay, $endMonth) {
        ...
        if ($this->dateMonthNoDay && !$startDay && !$endDay)
            $this->{$type}[$this->styleMap->{$type}['date']] = $this->dateArray[$this->styleMap->{$type}['date']];
        if ($startDay !== FALSE) {
