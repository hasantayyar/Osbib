...
// $output is 'html' or 'rtf'
    function __construct($db, $output, $rtfBibExport = FALSE) {
        $this->db = $db;
...
// 3rd order
        ...
        $resultset = $this->db->select(array("WKX_resource"), $stmt->listFields('creator'), $stmt->listJoin() . $join . $condition .
...
// When a user cut's 'n' pastes in HTML design mode, superfluous HTML tags (usually <style lang=xx></span>) are inserted.  Remove anything that looks like HTML
        ...
        $id = $idPart[0];
        $resultset = $this->db->select(array("WKX_resource"), array('id'), " WHERE " . $this->db->formatField('id') . "=" . $this->db->tidyInput($id));
...
// For an invalid citation ID, deal with any text that precedes it by either prepending to the next cite tag capture or prepending to $this->tailText.
            if (array_key_exists($matchIndex + 1, $this->matches[1]))
                $this->matches[1][$matchIndex + 1] = $this->matches[1][$matchIndex] . $this->matches[1][$matchIndex + 1];
            else
